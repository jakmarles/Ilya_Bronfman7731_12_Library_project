...
def loan():
    ...
    if request.method == "POST":
        ...
        tr = db.Loans(customer_id, book_id, loan_date, return_date)
        session.add(tr)
        session.commit()
...
if __name__ == '__main__':
    ...
    # runs the init db if the customer database is empty
    init_db.db_load_example_data_customers(app, db, session)
    # runs the init db if the book database is empty
    init_db.db_load_example_data_books(app, db, session)
